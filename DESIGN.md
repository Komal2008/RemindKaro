...
## RemindKaro Theme Toggle

- **Control:** `ThemeToggle` in landing header, site `Header`, dashboard topbar, and auth pages (`AuthThemeBar`).
- **Persistence:** `localStorage` key `remindkaro-theme`; inline script in `app/layout.js` prevents flash.
- **Attribute:** `data-theme="dark" | "light"` on `<html>` drives `--landing-*`, `--linear-*`, and `--color-*` overrides in `app/globals.css`.

## RemindKaro App Surfaces

The marketing spec above maps to **in-app auth and onboarding** in this repo. Landing hero **phone mockups** (`PhoneMockup`, `hero-col-center`) stay unchanged; auth flows use the Linear dark system via CSS variables in `app/globals.css` (`--linear-*`) and `app/styles/auth-flow.module.css`.

| Surface         | Route(s)           | Layout                 | Responsive notes                                                          |
| --------------- | ------------------ | ---------------------- | ------------------------------------------------------------------------- |
| Onboarding      | `/onboarding`      | Single card, 560px max | Safe-area padding; step padding tightens ≤768px; action row stacks ≤480px |
| Sign up         | `/signup`          | Two-column shell       | Brand panel stacks above form ≤900px; perks stay visible                  |
| Sign in         | `/login`           | Two-column shell       | Testimonial hidden on narrow brand panel (`brandPanelCompact`)            |
| Forgot password | `/forgot-password` | Single card, 520px max | Same touch targets and typography scale as onboarding                     |
| Reset password  | `/reset-password`  | Single card, 520px max | Matches forgot-password shell                                             |

**Implementation rules for app surfaces:**

- Use `{colors.primary}` / `--linear-primary` for CTAs and progress only — no gradient fills on buttons.
- Cards use `{colors.surface-1}` + `{colors.hairline}`; featured/hover states use surface-2/3.
- Display headings: weight 600, negative tracking per `{typography.headline}` / `{typography.card-title}`.
- Form inputs on dark surfaces: `Input` with `theme="dark"` (surface-2 background, primary-focus ring).
- Minimum tap height 44px on primary actions and selects (see Responsive Behavior).
